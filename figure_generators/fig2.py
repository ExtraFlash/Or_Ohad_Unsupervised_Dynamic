...
def plot_2d():
    ...
    colors_different_gases = ['green', 'darkorange', 'mediumblue', 'mediumvioletred', 'lightseagreen', 'indianred']

    clustering_different_colors = ['moccasin', 'lime', 'powderblue', 'lightgray', 'aquamarine', '#ff9e9e']
    colors_genders_dict = {}
    gases = list(range(1, 7))
    for i, gender in enumerate(gases):
        colors_genders_dict[gender] = colors_different_gases[i]

    samples_colors_genders_list = []
    for gender in y_test.tolist():
        # gender is float, convert to int
        gender = int(gender)
        # print(f'gender: {gender}, typeclass: {type(gender)}')
    ...
    plt.scatter(x=data_2d[:, 0], y=data_2d[:, 1], c=samples_colors_genders_list, marker='.', s=10)
    plt.xlabel("t-SNE Component 1", fontsize=16)
    plt.ylabel("t-SNE Component 2", fontsize=16)
    plt.savefig('../figures/Fig_kmeans_2', pad_inches=0.2, bbox_inches="tight")
    plt.show()
